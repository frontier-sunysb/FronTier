...
c  x     : double precision array of dimension at least (m).
c  y     : double precision array of dimension at least (m).
c  z     : double precision array of dimension at least (m).
...
c  w     : double precision array of dimension at least (m). before entry, w(i) must
c          be set to the i-th value in the set of weights. the w(i) must
c          be strictly positive. unchanged on exit.
c  rad   : double precision function subprogram defining the boundary of the approx-
c          imation domain, i.e   x = rad(v)*cos(v) , y = rad(v)*sin(v),
c          -pi <= v <= pi.
c          must be declared external in the calling (sub)program.
c  s     : double precision. on entry (in case iopt(1) >=0) s must specify the
...
c          in most practical situation nuest = nvest = 8+dsqrt(m/2) will
...
c          computer representation of a double precision number is q, then 10**(-q)
...
c  tu    : double precision array of dimension at least nuest.
...
c  tv    : double precision array of dimension at least nvest.
c          on succesful exit, this array will contain the knots of the
...
c          see also the restrictions (ier=10).
c  u     : double precision array of dimension at least (m).
...
c  c     : double precision array of dimension at least (nuest-4)*(nvest-4).
...
c  wrk1  : double precision array of dimension (lwrk1). used as workspace.
...
c  wrk2  : double precision array of dimension (lwrk2). used as workspace, but
...
c   z(i), a good s-value should be found in the range (m-dsqrt(2*m),m+
c   dsqrt(2*m)). if nothing is known about the statistical error in z(i)
...
c  ..
c  ..scalar arguments..
      double precision s,eps,fp
      integer m,nuest,nvest,nu,nv,lwrk1,lwrk2,kwrk,ier
c  ..array arguments..
      double precision x(m),y(m),z(m),w(m),tu(nuest),
     * tv(nvest),u(m),v(m),
...
c  ..local scalars..
      double precision tol,pi,dist,r,one
