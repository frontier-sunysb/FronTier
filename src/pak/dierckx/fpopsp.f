...
c  ..scalar arguments..
     ...
     * nc,lwrk
      double precision r0,r1,p,fp
c  ..array arguments..
      integer ider(4),nru(mu),nrv(mv),iopt(3)
      double precision u(mu),v(mv),r(mr),dr(6),tu(nu),tv(nv),
     * c(nc),fpu(nu),fpv(nv),wrk(lwrk),step(2)
c  ..local scalars..
      double precision res,sq,sqq,sq0,sq1,step1,step2,three
      integer i,id0,iop0,iop1,i1,j,l,lau,lav1,lav2,la0,la1,lbu,lbv,lb0,
     * lb1,lc0,lc1,lcs,lq,lri,lsu,lsv,l1,l2,mm,mvnu,number
      integer id1
c  ..local arrays..
      integer nr(6)
      double precision delta(6),drr(6),sum(6),a(6,6),g(6)
