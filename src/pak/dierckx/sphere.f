...
c  teta  : double precision array of dimension at least (m).
c  phi   : double precision array of dimension at least (m).
c  r     : double precision array of dimension at least (m).
...
c          in most practical situation ntest = npest = 8+dsqrt(m/2) will
...
c  tt    : double precision array of dimension at least ntest.
...
c          on entry.
c  tp    : double precision array of dimension at least npest.
c          on succesful exit, this array will contain the knots of the
...
c          see also the restrictions (ier=10).
c  c     : double precision array of dimension at least (ntest-4)*(npest-4).
...
c   r(i), a good s-value should be found in the range (m-dsqrt(2*m),m+
c   dsqrt(2*m)). if nothing is known about the statistical error in r(i)
...
c  ..array arguments..
     ...
     * c((ntest-4)*(npest-4)),wrk1(lwrk1),wrk2(lwrk2)
      integer iwrk(kwrk)
c  ..local scalars..
      double precision tol,pi,pi2,one
