...
c    v   : double precision array of dimension at least (m). before entry, v(i)
...
c    s   : double precision. on entry s must specify an over-estimate for the
...
c    sq  : double precision. unless ier>0 , sq contains the weighted sum of
c          squared residuals of the spline approximation returned.
c    sx  : double precision array of dimension at least m. on exit with ier<=0
...
c          calls.
c   wrk  : double precision array of dimension at least (m*4+nest*8+maxbin*(maxbin+
...
c  ..array arguments..
      double precision x(m),y(m),w(m),v(m),t(nest),
     * c(nest),sx(m),wrk(lwrk)
      integer iwrk(kwrk)
      logical bind(nest)
c  ..local scalars..
