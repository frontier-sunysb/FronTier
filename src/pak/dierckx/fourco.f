...
c
c  input parameters:
c    t    : double precision array,length n, containing the knots of s(x).
c    n    : integer, containing the total number of knots. n>=10.
c    c    : double precision array,length n, containing the b-spline coefficients.
c    alfa : double precision array,length m, containing the parameters alfa(i).
c    m    : integer, specifying the number of integrals to be computed.
c    wrk1 : double precision array,length n. used as working space
c    wrk2 : double precision array,length n. used as working space
c
c  output parameters:
c    ress : double precision array,length m, containing the integrals ress(i).
c    resc : double precision array,length m, containing the integrals resc(i).
c    ier  : error flag:
...
c  ..scalar arguments..
      integer n,m,ier
c  ..array arguments..
      double precision t(n),c(n),wrk1(n),wrk2(n),alfa(m),ress(m),resc(m)
c  ..local scalars..
      integer i,j,n4
      double precision rs,rc
c  ..
      n4 = n-4
