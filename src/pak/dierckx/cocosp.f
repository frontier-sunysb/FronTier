...
c    t   : double precision array of dimension at least (n). before entry, this
...
c    e   : double precision array of dimension at least (n). before entry, e(j)
...
c          unchanged on exit.
c    c   : double precision array of dimension at least (n).
c          on succesful exit, this array will contain the coefficients
c          c(1),c(2),..,c(n-4) in the b-spline representation of s(x)
c    sq  : double precision. on succesful exit, sq contains the weighted sum of
c          squared residuals of the spline approximation returned.
c    sx  : double precision array of dimension at least m. on succesful exit
...
c   wrk  : double precision array of dimension at least  m*4+n*7+maxbin*(maxbin+n+1)
...
c  ..array arguments..
      double precision x(m),y(m),w(m),t(n),e(n),c(n),sx(m),wrk(lwrk)
      integer iwrk(kwrk)
      logical bind(n)
c  ..local scalars..
