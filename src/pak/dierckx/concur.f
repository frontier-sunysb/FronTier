...
c   u     : double precision array of dimension at least (m). before entry,
...
c   x     : double precision array of dimension at least idim*m.
...
c   xx    : double precision array of dimension at least idim*m.
...
c           unchanged on exit.
c   db    : double precision array of dimension nb. before entry db(idim*l+j) must
...
c           unchanged on exit.
c   de    : double precision array of dimension ne. before entry de(idim*l+j) must
...
c   c     : double precision array of dimension at least (nest*idim).
...
c   cp    : double precision array of dimension at least 2*(k+1)*idim.
...
c   wrk   : double precision array of dimension at least m*(k+1)+nest*(6+idim+3*k).
...
c   x(i), a good s-value should be found in the range (m-dsqrt(2*m),m+
c   dsqrt(2*m)). if nothing is known about the statistical error in x(i)
...
c   where
c        q(i)= dsqrt(sum j=1,idim (xj(i)-xj(i-1))**2 )
...
c  ..array arguments..
      double precision u(m),x(mx),xx(mx),db(nb),de(ne),w(m),
     * t(nest),c(nc),wrk(lwrk)
      double precision cp(np)
      integer iwrk(nest)
c  ..local scalars..
