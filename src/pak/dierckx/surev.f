...
c   c     : double precision array, length (nu-4)*(nv-4)*idim, which contains the
c           b-spline coefficients.
c   u     : double precision array of dimension (mu).
...
c           the u-axis. mu >=1.
c   v     : double precision array of dimension (mv).
...
c
c  output parameters:
c   f     : double precision array of dimension (mf).
...
c  ..array arguments..
      integer iwrk(kwrk)
      double precision tu(nu),tv(nv),c((nu-4)*(nv-4)*idim),u(mu),
     * v(mv),f(mf),wrk(lwrk)
c  ..local scalars..
      integer i,muv
c  ..
