...
c  r     : double precision array of dimension at least (mu*mv).
c          before entry, r(mv*(i-1)+j) must be set to the data value at
c          the grid point (u(i),v(j)) for i=1,...,mu and j=1,...,mv.
c          unchanged on exit.
c  r0    : double precision value. on entry (if ider(1) >=0 ) r0 must specify the
c          data value at the pole u=0. unchanged on exit.
c  r1    : double precision value. on entry (if ider(1) >=0 ) r1 must specify the
...
c  ..local scalars..
      double precision per,pi,tol,uu,ve,rmax,rmin,one,half,rn,rb,re
