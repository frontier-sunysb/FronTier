...
c  ..array arguments..
      double precision x(m),y(m),w(m),t(nest),c(nest),
     * fpint(nest),z(nest),
     * a1(nest,k1),a2(nest,k),b(nest,k2),g1(nest,k2),g2(nest,k1),
     * q(m,k1)
      integer nrdata(nest)
c  ..local scalars..
      double precision acc,cos,c1,d1,fpart,fpms,fpold,fp0,f1,f2,f3,
     * p,per,pinv,piv,
...
c  ponding weights of these additional rows are set to 1/dsqrt(p).      c
