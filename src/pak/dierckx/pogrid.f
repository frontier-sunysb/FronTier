...
c  u     : double precision array of dimension at least (mu). before entry, u(i)
...
c  v     : double precision array of dimension at least (mv). before entry, v(j)
...
c  z     : double precision array of dimension at least (mu*mv).
c          before entry, z(mv*(i-1)+j) must be set to the data value at
c          the grid point (u(i),v(j)) for i=1,...,mu and j=1,...,mv.
c          unchanged on exit.
c  z0    : double precision value. on entry (if ider(1) >=0 ) z0 must specify the
c          data value at the origin. unchanged on exit.
c  r     : double precision value. on entry r must specify the radius of the disk.
c          r>=u(mu) (>u(mu) if iopt(3)=1). unchanged on exit.
c  s     : double precision. on entry (if iopt(1)>=0) s must specify the smoothing
...
c  ..
c  ..scalar arguments..
      double precision z0,r,s,fp
      integer mu,mv,nuest,nvest,nu,nv,lwrk,kwrk,ier
c  ..array arguments..
      integer iopt(3),ider(2),iwrk(kwrk)
      double precision u(mu),v(mv),z(mu*mv),c((nuest-4)*(nvest-4)),
     * tu(nuest),tv(nvest),wrk(lwrk)
c  ..local scalars..
      double precision per,pi,tol,uu,ve,zmax,zmin,one,half,rn,zb
