...
c  ..scalar arguments..
     ...
     * lasttu,lwrk,ier
      double precision z0,r,s,tol,fp,fp0,fpold,reducu,reducv,step
c  ..array arguments..
      integer iopt(3),ider(2),nrdatu(nuest),nrdatv(nvest),nru(mu),
     * nrv(mv)
      double precision u(mu),v(mv),z(mz),tu(nuest),tv(nvest),c(nc),
     * fpintu(nuest),fpintv(nvest),dz(3),wrk(lwrk)
c  ..local scalars..
      double precision acc,fpms,f1,f2,f3,p,per,pi,p1,p2,p3,vb,ve,
     * zmax,zmin,rn,one,con1,con4,con9
...
c  ..local arrays..
      integer idd(2)
      double precision dzz(3)
c  ..function references..
      double precision abs,datan2,fprati
      integer max0,min0
