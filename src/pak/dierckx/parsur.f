...
c          supplied in strictly ascending order. unchanged on exit.
c  f     : double precision array of dimension at least (mu*mv*idim).
...
c  s     : double precision. on entry (if iopt>=0) s must specify the smoothing
...
c          see also the restrictions (ier=10).
c  c     : double precision array of dimension at least (nuest-4)*(nvest-4)*idim.
...
c  ..array arguments..
      double precision u(mu),v(mv),f(mu*mv*idim),tu(nuest),tv(nvest),
     * c((nuest-4)*(nvest-4)*idim),wrk(lwrk)
      integer ipar(2),iwrk(kwrk)
c  ..local scalars..
      double precision tol,ub,ue,vb,ve,peru,perv
