...
c
c  input parameters:
c    t    : double precision array,length n, containing the knots.
c    n    : integer, containing the number of knots.
c    par  : real, containing the value of the parameter par.
c
c  output parameters:
c    ress  : double precision array,length n, containing the integrals ress(j).
c    resc  : double precision array,length n, containing the integrals resc(j).
c
c  restrictions:
...
c  ..array arguments..
      double precision t(n),ress(n),resc(n)
c  ..local scalars..
      integer i,ic,ipj,is,j,jj,jp1,jp4,k,li,lj,ll,nmj,nm3,nm7
      double precision ak,beta,con1,con2,c1,c2,delta,eps,fac,f1,f2,f3,
     * one,quart,sign,six,s1,s2,term
c  ..local arrays..
      double precision co(5),si(5),hs(5),hc(5),rs(3),rc(3)
c  ..function references..
      double precision cos,sin,abs
c  ..
c  initialization.
      one = 0.1d+01
      six = 0.6d+01
