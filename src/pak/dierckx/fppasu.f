...
c  ..
c  ..scalar arguments..
      double precision s,tol,fp,fp0,fpold,reducu,reducv
      integer iopt,idim,mu,mv,mz,nuest,nvest,maxit,nc,nu,nv,lastdi,
     * nplusu,nplusv,lwrk,ier
c  ..array arguments..
      double precision u(mu),v(mv),z(mz*idim),tu(nuest),
     * tv(nvest),c(nc*idim),
     * fpintu(nuest),fpintv(nvest),wrk(lwrk)
      integer ipar(2),nrdatu(nuest),nrdatv(nvest),nru(mu),nrv(mv)
c  ..local scalars
      double precision acc,fpms,f1,f2,f3,p,p1,p2,p3,rn,one,
     * con1,con9,con4,peru,perv,ub,ue,vb,ve
