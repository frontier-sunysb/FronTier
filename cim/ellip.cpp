...
void CIM_ELLIPTIC_SOLVER::solve2d(double *soln)
{
        ...
        for (i = 0; i < 2*dim; ++i) 
	{
        ...
        FT_FreeThese(2,b,Order);
        FT_FreeThese(3,D,u.u,V);
}	/* end solve2d */
...
{
    	int ii,jj,i,j,k,index,SD[3],ic[MAXD],ic2[MAXD];
...
{
    	int i,j,n,k,ii,jj,index,d,s,N,Ri;
    	int *row, *col;
	...
	for (jj = jmin; jj <= jmax; ++jj)
	{
		...
		for (i = 0; i < 2*dim; ++i) 
		{
			...
			b[k] += 1.0/h[d]/h[d]*epi*getStateVar(state);
		    }
		    v += 1.0/h[d]/h[d]*epi;
		}
		row[n] = col[n] = k;
